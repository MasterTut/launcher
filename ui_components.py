...
background_img= pygame.image.load("./Assets/background2.jpg").convert()
...
class TextField:
    ...
    def display(self):
        if self.layer is None:
            print(f"Error: No layer set for text field {self.label}")
            return
        # Draw background
        pygame.draw.rect(self.layer, (50, 50, 50), self.buttonRect)
        # Draw border (highlight if selected)
        border_color = (255, 255, 0) if self.isSelected else (255, 255, 255)
        pygame.draw.rect(self.layer, border_color, self.buttonRect, 2)
        # Draw label and text
        label_surface = self.font.render(f"{self.label}:", True, (255, 255, 255))
        text_surface = self.font.render(self.text, True, (255, 255, 255))
        self.layer.blit(label_surface, (self.x + 5, self.y + 5))
        self.layer.blit(text_surface, (self.x + 100, self.y + 5))

    def handle_input(self, event):
        if not self.isSelected or not self.is_editing:
            return
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_BACKSPACE:
                self.text = self.text[:-1]
            elif event.key == pygame.K_RETURN:
                self.is_editing = False  # Stop editing on Enter
            else:
